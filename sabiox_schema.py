...
def empty_payload() -> Dict[str, Any]:
    """Estrutura oficial SABiOx sem campos redundantes."""
    return {
...
def sanitize_requirements(data: Any) -> Dict[str, Any]:
    """Limpa o JSON e garante que os RFs fiquem apenas nos Subdomínios."""
    out = empty_payload()
    ...
    # Subdomains e suas respectivas RFs
    ...
    if isinstance(src_sub, list):
        for sub in src_sub:
            if isinstance(sub, dict):
                ...
                # Pega as RFs que o Regex ou a IA colocaram dentro do subdomínio
...
def validate_requirements(data: Dict[str, Any]) -> Tuple[bool, List[str]]:
    """Valida se os campos obrigatórios da Fase de Requisitos foram preenchidos."""
    errs: List[str] = []
